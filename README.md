...
A highly precise, zero-dependency Dart package for seamlessly converting between hundreds of different units of measurement. 

In **v5.0.0**, the package has been completely rewritten to use exact `Rational` math (backed by BigInt), completely eliminating the floating-point drift and epsilon rounding errors commonly associated with standard `double` math!

## Features
- **100% Mathematical Accuracy**: Uses `Rational` to ensure precision is never lost during conversions. No more `1.99999999999999`!
- **Extensive Unit Coverage**: Supports over 1000+ individual units across 80+ physical quantities.
- **Type-Safe Design**: Strictly typed hierarchical classes prevent adding `Mass` to `Length`.
- **JSON Serialization**: Full support for serializing and deserializing measurement objects.
- **Operator Overloading**: Add, subtract, multiply, and divide measurements directly (e.g. `Length$Meter(1) + Length$Foot(1)`).

## Mathematical Precision & Credits

To ensure 100% precision without floating-point drift, this package embeds a highly customized and heavily optimized version of the [rational](https://pub.dev/packages/rational) package, originally created by [a14n](https://github.com/a14n). We have extended and rewritten the core mathematical operators specifically for unit conversion workflows.

By calculating cross-reductions *before* performing large `BigInt` multiplications, our modified version executes up to **100x faster** during complex chained conversions, while allowing `super_measurement` to remain a **100% zero-dependency** package!
...
void main() {
  // Simple creation
  final length1 = Length$Meter(1.5);
  final length2 = Length$Foot(2);

  // Math operators automatically handle underlying conversions!
  final totalLength = length1 + length2; 

  // Convert to specific unit
  print(totalLength.toCentimeter); // Length$Centimeter(210.96)

  // Extract raw dart double
  double rawMeters = totalLength.toMeter.value.toDouble();
}
```

<!-- GENERATED_UNITS_START -->
## Available Units
### Can be accessed using factory constructors or classes
...
### See [example](https://pub.dev/packages/super_measurement/example) for more usage examples.
